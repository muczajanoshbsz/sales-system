...
import { ShieldAlert, Clock, LogOut, RefreshCw, AlertTriangle, Lock } from 'lucide-react';
...
const sessionChannel = new BroadcastChannel('airpods_vault_session');
...
export const SessionMonitor: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        ...
        const sessionConfig = configs.find(c => c.key === 'SESSION_TIMEOUT_MINUTES');
        if (sessionConfig && sessionConfig.value && !isNaN(parseInt(sessionConfig.value))) {
          const mins = Math.max(5, Math.min(60, parseInt(sessionConfig.value)));
  ...
  const handleLogout = useCallback(async () => {
    ...
    // Draft Protection: Save any generic draft data to session storage
    const drafts: Record<string, any> = {};
    // This is a placeholder for where we'd iterate over important form inputs if we had a global registry
    // For now, we rely on the fact that we don't clear localStorage/sessionStorage on standard signOut
  ...
  const resetTimers = useCallback((broadcastNotification = true) => {
    ...
    // Timing calculations:
    // We start blurring 5 minutes before logout, but never less than 2 minutes into the session
    const blurDelay = Math.max(2 * 60 * 1000, timeoutDuration - 5 * 60 * 1000);
    const warningDelay = timeoutDuration - 60 * 1000;
      ...
      <AnimatePresence>
        {isBlurred && !isWarning && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm cursor-pointer"
            onClick={() => resetTimers()}
          >
            <motion.div 
        ...
        {isWarning && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-xl">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-[0_32px_64px_-12px_rgba(220,38,38,0.4)] border border-red-500/30 w-full max-w-md overflow-hidden"
            >
