...
async function startServer() {
  ...
  async function sendWeeklyReportEmail(reportData: any, reportText: string, startDate: Date, endDate: Date) {
    ...
    try {
      console.log(`✉️ Attempting to send report email via smtp.gmail.com (STARTTLS, IPv4 Forced)...`);
      const transporter = nodemailer.createTransport({
        ...
        socketTimeout: 20000,
        family: 4, // Absolute IPv4 force to avoid network ENETUNREACH
        tls: {
          rejectUnauthorized: false,
  ...
  async function sendSystemEmail(subject: string, title: string, content: string, details?: any) {
    ...
    try {
      console.log(`✉️ Attempting to send system email via smtp.gmail.com (STARTTLS, IPv4 Forced)...`);
      const transporter = nodemailer.createTransport({
        host: 'smtp.gmail.com',
        port: 587,
        secure: false, // Use STARTTLS
